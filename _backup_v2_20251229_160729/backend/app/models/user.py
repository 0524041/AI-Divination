...
def get_user_api_key_info(user_id: int, provider: str) -> dict:
    ...
    config = {}
    if row['config_json']:
        print(f"[DEBUG get_user_api_key_info] config_json exists: {row['config_json'][:50]}...")
        try:
            config = json.loads(row['config_json'])
            print(f"[DEBUG get_user_api_key_info] parsed config: {config}")
        except Exception as e:
            print(f"[DEBUG get_user_api_key_info] JSON parse error: {e}")
            config = {}
    else:
        print(f"[DEBUG get_user_api_key_info] config_json is empty")
