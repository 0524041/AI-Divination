...
def format_divination_result(data: dict, gender: str = None, target: str = None) -> str:
    ...
    lines.append(f"卦類型: {data.get('gua_type', '未知')}")
    ...
    if 'gua_ci' in data:
        ...
        lines.append("【易經原文】")
        lines.append(dash)
        lines.append(f"卦辭: {data['gua_ci']}")
        
        yao_ci = data.get('yao_ci', {})
        if yao_ci:
            lines.append("")
            lines.append("爻辭:")
            for i in range(6, 0, -1):  # 從上爻到初爻
                if i in yao_ci:
                    lines.append(f"  {yao_ci[i]}")
        lines.append(sep)
...
def call_ai(prompt: str, provider: str = 'local', user_gemini_key: str = None, user_local_config: dict = None) -> tuple[str, str]:
    ...
    if provider == 'gemini':
        ...
        if user_local_config and user_local_config.get('api_url') and user_local_config.get('model_name'):
            local_url = user_local_config['api_url']
            local_model = user_local_config['model_name']
            print(f"[AI Service] Using USER Local AI: {local_model} at {local_url}")
        else:
            # Fallback 到全域設定（但通常不應該到這裡，因為前面已經檢查）
            local_url = get_setting('local_api_url', 'http://localhost:1234/v1')
            local_model = get_setting('local_model_name', 'qwen/qwen3-8b')
            print(f"[AI Service] WARNING: Using GLOBAL Local AI (user config missing): {local_model} at {local_url}")
