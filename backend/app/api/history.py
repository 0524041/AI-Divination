...
def get_history(
    ...
    if current_user.role == "guest":
        return HistoryListResponse(items=[], total=0, page=1, page_size=page_size)
...
def get_statistics(
    ...
    if user_id is not None:
        ...
        if current_user.role != "admin":
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="無權限查看其他用戶的統計"
            )
        if user_id == 0:
            # 查看所有用戶
    ...
    if query_all_users:
        ...
        total_count = (
            db.query(History).filter(History.user_id == target_user_id).count()
        )
    ...
    if query_all_users:
        ...
        today_count = (
            db.query(History)
            .filter(
                History.user_id == target_user_id, History.created_at >= today_start
            )
            .count()
        )
    ...
    if query_all_users:
        type_counts = (
            ...
            .group_by(History.divination_type)
            .all()
        )
    else:
        type_counts = (
            db.query(History.divination_type, func.count(History.id).label("count"))
            .filter(
                History.user_id == target_user_id, History.created_at >= seven_days_ago
            )
            .group_by(History.divination_type)
            .all()
        )
    ...
    # 找出最常使用的類型
    most_used_type = (
        max(type_counts_dict.items(), key=lambda x: x[1])[0]
        if type_counts_dict
        else "無"
    )
...
def create_share_token(
    ...
    """建立分享連結（需登入）"""
    print(
        f"[DEBUG] create_share_token called. User: {current_user.id} ({current_user.username}), Role: {current_user.role}, HistoryID: {request.history_id}"
    )
    ...
    if current_user.role != "admin":
        print(
            f"[DEBUG] User is not admin. Applying filter: user_id == {current_user.id}"
        )
    ...
    if not history:
        print(
            f"[ERROR] Share failed: History not found. User {current_user.id} ({current_user.role}) tried to share History {request.history_id}"
        )
    ...
    share_token = ShareToken(
        token=token, history_id=request.history_id, expires_at=expires_at
    )
...
def get_shared_history(token: str, db: Session = Depends(get_db)):
    """取得分享的歷史紀錄（公開存取，不需登入）"""
    # 查詢 token
    ...
    if not share_token:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="分享連結不存在"
        )

    # 檢查是否過期
    if share_token.is_expired():
        raise HTTPException(status_code=status.HTTP_410_GONE, detail="分享連結已過期")

    # 取得歷史紀錄
    history = db.query(History).filter(History.id == share_token.history_id).first()
