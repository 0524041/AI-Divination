...
export function Navbar({
    ...
    return (
        <>
            ...
            <nav
                className={cn(
                    'sticky top-0 bg-background-card/80 backdrop-blur-md border border-border-accent/50 rounded-2xl mx-4 px-4 sm:px-6 py-4 flex items-center justify-between z-50 shadow-sm transition-colors duration-300',
                    className
                )}
            >
            ...
            {/* Mobile Menu */}
            {menuOpen && (
                <div className="md:hidden fixed inset-x-0 top-[calc(4rem+8px)] mx-4 bg-background-card/95 backdrop-blur-xl border border-border-accent/50 rounded-2xl p-4 space-y-4 animate-in fade-in slide-in-from-top-2 z-40 shadow-lg">
                    {navItems.map((item) => (
                        <Link
                            key={item.href}
                            href={item.href}
