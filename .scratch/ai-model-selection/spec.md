...
- **OpenCode Go 相容性**（docs/zh-tw/go）：同一 base_url 下模型分屬三種協定——`/v1/chat/completions`（GLM、Kimi、LongCat、DeepSeek、MiMo、Hy 系列）✅ 支援；`/v1/responses`（OpenAI Responses API：Grok 4.6、GPT 5.6 Luna、Muse Spark）✅ 支援（模型層 `protocol: "responses"`，provider 依此分流 payload 與事件解析，preset 建議清單自動標記）；`/v1/messages`（Anthropic 型：MiniMax、Qwen3.x）❌ 不支援。`GET /v1/models` 無需授權即可列出。
- **已知風險**：
