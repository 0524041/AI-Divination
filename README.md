...
### v1.4.3 (2026-01-26) 🔮
- **UI 重構**：
  - **六爻占卜**：全新設計的視覺化卦象盤面 (LiuyaoChart)。
    - **純文字升級**：將原有的純文字結構改為現代化的表格呈現。
    - **視覺優化**：移除冗餘圖形，採用緊湊排版，支援動爻紅字呼吸燈提示與世應高亮標記。
    - **資訊整理**：清晰的起卦時間、干支神煞資訊卡片，解決跑版問題。

### v1.4.2 (2026-01-26) 🔮
- **UI 修復與優化**：
  - **紫微斗數**：修正命盤背景色，將原本的紫色底改為與主題一致的深色 (#0D1117)，提升整體協調性。
  - **輸入介面**：修復輸入框 (Input/Select) 在深色模式下對比度不足的問題，採用半透明背景確保文字清晰可見。
